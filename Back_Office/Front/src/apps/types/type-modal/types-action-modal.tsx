...
interface TypesActionModalProps {
  typ?: Type;
  open: boolean;
  onClose: () => void;
...
export function TypesActionModal({
  typ,
  open,
  onClose,
  mode,
  ...
  function onSubmit(data: FormData) {
    if (isEdit && typ) {
      ...
      createType(data, { onSuccess: handleClose });
    }
  }
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-4">
            <p className="text-muted-foreground">
              {t("delete_type_confirmation", { name: typ?.name })}
            </p>
          </div>
          <DialogFooter>
