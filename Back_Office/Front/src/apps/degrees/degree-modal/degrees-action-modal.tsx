...
import { useCreateDegree, useUpdateDegree } from "@/hooks/use-degrees";
...
const formSchema = z.object({
  name: z
    ...
    .min(3, { message: "Degree name must be at least 3 characters long." }),
});

type FormData = z.infer<typeof formSchema>;

interface DegreesActionModalProps {
  degree?: Degree;
  open: boolean;
  onClose: () => void;
...
export function DegreesActionModal({
  degree,
  open,
  onClose,
  mode,
  ...
  function onSubmit(data: FormData) {
    if (isEdit && degree) {
      ...
      createDegree(data, { onSuccess: handleClose });
    }
  }
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-4">
            <p className="text-muted-foreground">
              {t("delete_degree_confirmation", { name: degree?.name })}
            </p>
          </div>
          <DialogFooter>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="grid grid-cols-6 items-center space-y-0 gap-x-4 gap-y-1">
                  ...
                  <FormControl>
                    <Input
                      placeholder={t("placeholder_degree_name")}
